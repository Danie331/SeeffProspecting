...
-- get matching records which dont have a lightstone_reg_date in Prospecting
-- Option: update the latest reg date to the value in base data.
select bd.property_id from ls_base.dbo.base_data bd
 join seeff_prospecting.dbo.prospecting_property pp on bd.property_id = pp.lightstone_property_id
where pp.lightstone_reg_date = ''
...
select * into #temp from cte2 where rn = 1;
update pp
