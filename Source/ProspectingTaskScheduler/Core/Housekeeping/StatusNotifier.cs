...
namespace ProspectingTaskScheduler.Core.Housekeeping
{
    public class StatusNotifier
    {
        ...
        public static void SendYesterdaysLightstoneCallLog(IJobCancellationToken cancellationToken)
        {
            try
            {
            ...
            {
                return; // retry gracefully
            }
            catch (Exception ex)
            {
                SendEmail("danie.vdm@seeff.com", "ProspectingTaskScheduler", "reports2@seeff.com", null, "Exception in SendYesterdaysLightstoneCallLog()", ex.ToString());
            }
        }
