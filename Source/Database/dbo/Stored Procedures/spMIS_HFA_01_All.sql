...
-- Description:	Hub Field Analysis
--				Returns all the records
--				No paramaters
-- =============================================
CREATE PROCEDURE  [dbo].[spMIS_HFA_01_All]
...
ON [dbo].[sps_transaction] ([sps_reporting_date])
...
WHERE     
			(sps_transaction.sps_sold_date IS NULL)
OR
			(sps_transaction.sps_refferal_type <> 'External paid to you')
OR
			(sps_transaction.sps_selling_price IS NULL)	
OR
			(dbo.credited_agents(sps_transaction.sps_transaction_ref) <> '(NO AGENT R0.00)')
OR
			(sps_expected_reg_date IS NULL)
OR
			(dbo.person_by_roll(sps_transaction.sps_transaction_ref, 'seller')) IS NULL	
OR
			(dbo.person_by_roll(sps_transaction.sps_transaction_ref, 'buyer')) IS NULL
OR
			(sps_transaction.sps_comment = '')	
OR
			(sps_reporting_date IS NULL)														
...
HAVING
			(sps_transaction.sps_reporting_date IS NOT NULL)
AND
			user_registration_1.registration_id = @DealMakerID						
...
DROP INDEX Indx02 ON [dbo].[sps_transaction]			
END
