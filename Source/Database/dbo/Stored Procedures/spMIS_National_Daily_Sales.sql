...
-- Description:	Stored procedure to create the
--				National Daily Sales Report
--				2012-03-02: Changed comms perc
--							calculation
-- =============================================
CREATE PROCEDURE [dbo].[spMIS_National_Daily_Sales]
...
SELECT   
			 ...
			 ,RIGHT(CONVERT(VARCHAR(10), sps_tran.sps_reporting_date, 105), 7) AS [Year_Month_Reported_On]
...
GROUP BY 
			...
			, RIGHT(CONVERT(VARCHAR(10), sps_tran.sps_reporting_date, 105), 7)
			, license.region
			
UNION ALL
		SELECT
			 '01No Sales Recorded' AS [Division]
			 ,RIGHT(CONVERT(VARCHAR(10), @DateEnd, 105), 7) AS [Year_Month_Reported_On]
			 ,lic.region AS [Region]
			 ,lic.license_name AS [License]
			 , 0 AS [Unit Count]
			 ,0 as [Total_Actual_Selling] 
			 ,0 as [Sales_At_Seven_Perc]
			 ,0 as [Comm_Perc]
			 ,0 as [Total_Comm]
		FROM         
			license as lic
		WHERE
			lic.license_id NOT IN (
									   SELECT 
												license.license_id 
									   FROM         
												sps_transaction AS sps_tran
...
ORDER BY 
			Division DESC, Sales_At_Seven_Perc DESC, Region, license 
