...
-- =============================================
CREATE FUNCTION [dbo].[credited_agents] 
(
...
BEGIN
    ...
    SELECT @details = COALESCE(@details + ', ', '') + '(' + (SELECT CASE [registration_id] WHEN 1667 THEN 'NO AGENT' ELSE [user_preferred_name] + ' ' + [user_surname] END  
                                                              ...
                                                              WHERE [boss].[dbo].[user_registration].[registration_id] = [boss].[dbo].[sps_agent_split].[registration_id]) + ' R'+ Convert(varchar,Convert(money,comm_paid),1) + 
