...
CREATE procedure [dbo].[sp_update_area_layer_tbl] @target_area_id int
as
begin
	declare @area_id int;
	...
	declare area_area_type_cursor cursor for 
		...
		WHERE [t0].[area_id] = @target_area_id
	...
	BEGIN
		...
		if exists (select area_id from area_layer where area_id = @area_id and area_type = @area_type and province_id = @prov_id)
		begin
			update area_layer 
			set formatted_poly_coords = @poly_coords
			where area_id = @area_id and area_type = @area_type and province_id = @prov_id;
		end
		else
		begin
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_update_area_layer_tbl] TO [IIS APPPOOL\smartadmin.seeff.com]
    AS [dbo];
